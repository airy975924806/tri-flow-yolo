...
from models.commonv5 import Classifyy,Classify,DaulConcat,CC
try:
    ...
    def fuseforward(self, x):
        ...
        for i in range(self.nl):
            ...
            if not self.training:  # inference
                ...
                if not torch.onnx.is_in_onnx_export():
                    y[..., 0:2] = (y[..., 0:2] * 2. - 0.5 + self.grid[i]) * self.stride[i]  # xy
                    y[..., 2:4] = (y[..., 2:4] * 2) ** 2 * self.anchor_grid[i]  # wh # tc
                else:
                    xy = (y[..., 0:2] * 2. - 0.5 + self.grid[i]) * self.stride[i]  # xy
...
class IBin(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            x[i] = self.m[i](self.ia[i](x[i]))  # conv
            x[i] = self.im[i](x[i])
            bs, _, ny, nx = x[i].shape  # x(bs,255,20,20) to x(bs,3,20,20,85) # tc
            x[i] = x[i].view(bs, self.na, self.no, ny, nx).permute(0, 1, 3, 4, 2).contiguous()
...
class Model(nn.Module):
    def __init__(self, cfg='yolor-csp-c.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        if isinstance(m, Detect):
            ...
            # print('Strides: %s' % m.stride.tolist()) # tc
    ...
    def forward_once(self, x, profile=False):
        ...
        for m in self.model:
            ...
            x = m(x)  # run
            if isinstance(m,(Classifyy,Classify)):
                k.append(x)
            if isinstance(m,CC):
                l.append(x)
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        n = max(round(n * gd), 1) if n > 1 else n  # depth gain # tc
        if m in [nn.Conv2d, Conv, RobustConv, RobustConv2, DWConv, GhostConv, RepConv, RepConv_OREPA, DownC, 
            ...
            c2 = ch[f] // args[0] ** 2
        elif m in [Classify,Classifyy,CC]:
            c1,c2 = ch[f],args[0]
            args = [c1,c2]
        elif m is DaulConcat:
