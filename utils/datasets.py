    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            if random.random() < 0.8:
                ...
                img,labels = getitem_multi(self,index)
            else:
...
def getitem_multi(self, idx, n=2):
        hyp = self.hyp
        mosaic_labels = []
        # input_dim = self.img_size
        input_h, input_w = self.img_size,self.img_size
        # print('input_dim',input_h)
        indices = [idx] + [random.randint(0, self.n - 1) for _ in range(n * n - 1)]
        ...
        for i in range(n):
            for j in range(n):
                if i == j == 0:
                    ...
                    img, _labels = load_mosaic(self,indices[0])
                else:
                    img, _labels = load_mosaic(self,indices[i * n + j])
                ...
                try:
                    ...
                    print(mosaic_img[y_n:y_n + h, x_n:x_n + w].shape, mosaic_img.shape, img.shape, h, w, i, j, input_h,
                          input_w, y_n, x_n)
                    import time
                    time.sleep(1000)
                ...
                if _labels.size > 0:
                    labels[:, 1] = scale * _labels[:, 1] + input_w * i
                    labels[:, 2] = scale * _labels[:, 2] + input_h * j
                    labels[:, 3] = scale * _labels[:, 3] + input_w * i
                    labels[:, 4] = scale * _labels[:, 4] + input_h * j
                    labels[:,1:5] *= 1.0 / n
