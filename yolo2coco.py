...
coco_format_save_path='/home/airy/Downloads/cityscapes/yolo_format_8class/'                      #要生成的标准coco格式标签所在文件夹
yolo_format_classes_path='/home/airy/Downloads/cityscapes/yolo_format_8class/classes.txt'     #类别文件，一行一个类
yolo_format_annotation_path='/home/airy/Downloads/cityscapes/yolo_format_8class/labels/val_foggy/'        #yolo格式标签所在文件夹
img_pathDir='/home/airy/Downloads/cityscapes/yolo_format_8class/images/val_foggy/'                        #图片所在文件夹
...
for j,label in enumerate(lines1):
    label=label.strip()
    categories.append({'id':j,'name':label})         #将类别信息添加到categories中
# print(categories)

write_json_context=dict()                                                      #写入.json文件的大字典
# write_json_context['info']= {'description': '', 'url': '', 'version': '', 'year': 2021, 'contributor': '', 'date_created': '2023-03-07'}
...
for i,imageFile in enumerate(imageFileList):
    ...
    img_context['id']= imageFile[:-4]                                                        #该图片的id
...
name = os.path.join(coco_format_save_path,"val_foggy"+ '.json')
